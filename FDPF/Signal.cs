...
using System.Collections.Generic;
    ...
    class Signal
    {
        ...
        {
            paramValues = updateFunc(paramValues);
        ...
        Func<double[], double[]> transform_delta = (xs) =>
        {

            return (Array.IndexOf(xs, 0) == -1 && xs[0] < 0 && xs[xs.Length - 1] > 0) ?
            new List<double>(VectorOperation.PushElement(xs, 0)).GetRange(1, xs.Length-1).ToArray()
            : xs;
        };
            name = "delta(x)";
            paramNames = new string[] { };
            ...
            Func<double, double[], double> func_rp =
                (x, ps) => (x > ps[1]) && (x < ps[2] * (1/ps[0]) + ps[1]) ?
                Math.Sin(2 * Math.PI * (x-ps[1]) * ps[0] ) : 0;
